...
export const api = {
  ...
  botCreate: (body) => request('/bots', { method: 'POST', body }),
  botUpdate: (id, body) => request(`/bots/${id}`, { method: 'PUT', body }),
  botToggle: (id, enabled) => request(`/bots/${id}/toggle`, { method: 'POST', body: { enabled } }),
  botDelete: (id) => request(`/bots/${id}`, { method: 'DELETE' }),
  botExample: () => request('/bots/example', { method: 'POST' }),
  botCheck: (id) => request(`/bots/${id}/check`),
  botSaveNode: (id, key, body) => request(`/bots/${id}/nodes/${key}`, { method: 'PUT', body }),
  botDeleteNode: (id, key) => request(`/bots/${id}/nodes/${key}`, { method: 'DELETE' }),
  botTry: (id, body) => request(`/bots/${id}/try`, { method: 'POST', body }),
  botLayout: (id, nodes) => request(`/bots/${id}/layout`, { method: 'PUT', body: { nodes } }),
  botConnect: (id, body) => request(`/bots/${id}/connect`, { method: 'PUT', body }),
  sheetsStatus: () => request('/bots/sheets/status'),
  sheetsHeaders: (body) => request('/bots/sheets/headers', { method: 'POST', body }),
  botConversations: (id) => request(`/bots/${id}/conversations`),
  botTranscript: (id, chatRowId) => request(`/bots/${id}/conversations/${chatRowId}`),
